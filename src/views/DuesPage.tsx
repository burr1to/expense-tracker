...
export function DuesPage({ currency, items, customCategories, onSave, onDelete, onRecordPayment, onComplete, focusedId, focusedAction }: Props) {
...
function DueCard({ item, currency, customCategories, focused, startRepayment, onEdit, onDelete, onRecordPayment, onComplete }: { item: DueItem; currency: CurrencyCode; customCategories: CustomCategory[]; focused: boolean; startRepayment: boolean; onEdit: () => void; onDelete: (id: string) => Promise<void>; onRecordPayment: Props["onRecordPayment"]; onComplete: Props["onComplete"] }) {
  const [repaying, setRepaying] = useState(startRepayment); const [amount, setAmount] = useState<string | number>(dueRemaining(item) / 100); const [date, setDate] = useState(today()); const [note, setNote] = useState(""); const [addToLedger, setAddToLedger] = useState(true); const [busy, setBusy] = useState(false); const [error, setError] = useState<string | null>(null);
