import { FlagBanner, Sparkle, TrendDown, TrendUp } from "@phosphor-icons/react";
...
import type { CurrencyCode, CustomCategory, DueItem, LedgerTransaction } from "../types";
...
export function ReportsPage({ month, currency, transactions, customCategories, dueItems, onMonthChange, onAdd }: ReportsPageProps) {
    ...
    <div className="page reports-page">
      ...
      <section className="reports-grid">
        ...
        <article className="report-panel history-report">
          ...
          {history.length ? <div className="history-chart"><ResponsiveContainer width="100%" height="100%"><BarChart data={history} barGap={4}><CartesianGrid vertical={false} strokeDasharray="3 3" stroke="#dedbd4" /><XAxis dataKey="month" axisLine={false} tickLine={false} /><YAxis tickFormatter={(value) => formatMoney(Number(value), currency, true)} axisLine={false} tickLine={false} width={46} /><Tooltip cursor={false} formatter={(value) => formatMoney(Number(value), currency)} /><Bar dataKey="income" name="Income" fill="#2b8a50" radius={[5, 5, 0, 0]} isAnimationActive={false} /><Bar dataKey="expenses" name="Expenses" fill="#e8675a" radius={[5, 5, 0, 0]} isAnimationActive={false} /></BarChart></ResponsiveContainer></div> : <EmptyState />}
