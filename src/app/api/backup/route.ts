...
async function buildBackup(userId: string) {
  const db = getPrisma();
  const [user, categories, savedPlaces, accounts, transactions, transfers, budgets, recurring, goals, dues, receipts, receiptScans] = await Promise.all([
  ...
  const records: BackupRecord[] = [
    ...
    ...recurring.map((item): BackupRecord => ({ entity: "recurring_entry", backupId: item.id, payload: { kind: item.kind, category: item.category, amountMinor: item.amountMinor, note: item.note, tags: item.tags, dayOfMonth: item.dayOfMonth, nextDueOn: dateOnly(item.nextDueOn), active: item.active, createdAt: iso(item.createdAt), updatedAt: iso(item.updatedAt) } })),
...
async function restoreBackup(userId: string, csv: string) {
  ...
  try {
    await db.$transaction(async (transaction) => {
      ...
      if (accounts.length) await transaction.paymentAccount.createMany({ data: accounts.map(({ backupId, payload }) => ({ id: accountIds.get(backupId)!, userId, type: payload.type, provider: payload.provider, label: payload.label, balanceMinor: payload.balanceMinor, balanceAsOf: asDate(payload.balanceAsOf), balanceRecordedAt: asDateTime(payload.balanceRecordedAt), createdAt: asDateTime(payload.createdAt), updatedAt: asDateTime(payload.updatedAt) })) });
      ...
      if (recurring.length) await transaction.recurringEntry.createMany({ data: recurring.map(({ backupId, payload }) => ({ id: recurringIds.get(backupId)!, userId, kind: payload.kind, category: categoryId(payload.category), amountMinor: payload.amountMinor, note: payload.note, tags: payload.tags, dayOfMonth: payload.dayOfMonth, nextDueOn: asDate(payload.nextDueOn), active: payload.active, createdAt: asDateTime(payload.createdAt), updatedAt: asDateTime(payload.updatedAt) })) });
